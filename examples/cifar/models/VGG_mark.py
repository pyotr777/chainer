...
class VGG_mark(chainer.Chain):
    ...
    def __init__(self, class_labels=10):
        ...
        with self.init_scope():
            ...
            self.LN1 = L.LayerNormalization(None)

            # self.SUM = cp.ElementwiseKernel(
            #     'T x, T y',
            #     'T z',
            #     'z = x + y',
            #     'SUMgeneric')
    ...
    def __call__(self, x):
        ...
        # 512 channel blocks:
        # print("1 h: {}".format(h.shape))  # (160, 256, 4, 4)
        shape = h.shape
        h = cp.reshape(h,(h.shape[0],h.shape[1]*h.shape[2]*h.shape[3]))
        h = self.LN1(h)

        h = cp.reshape(h,shape)
        h = self.block4_1(h)
        h = F.dropout(h, ratio=0.4)
        h = self.block4_2(h)
        h = F.dropout(h, ratio=0.4)
